"use client";
// app/TaxiStand/components/ForPartnersSection.tsx
...
const partnerSteps = [
  {
    number: "01",
    title: "Download",
    description: "Download our app now to seamlessly access a world of convenient and affordable travel. Your next journey is just a tap away!",
    icon: "download",
  },
  {
    number: "02",
    title: "Register",
    description: "Join the Traveling Partner community swiftly. Register, verify your mobile number, and unlock a world of convenience.",
    icon: "register",
  },
  {
    number: "03",
    title: "Activate",
    description: "Activate your account by providing your necessary personal details. Enjoy personalized services, enhanced security, and a more connected journey.",
    icon: "activate",
  },
];
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-10 mb-20 perspective-1000">
          {partnerSteps.map((step, index) => (
            <PartnerCard key={step.number} step={step} index={index} />
          ))}
        </div>
...
function PartnerCard({ step, index }: { step: typeof partnerSteps[0]; index: number }) {
    ...
    <div className={`group relative ${index === 1 ? "md:-mt-8" : ""}`}>
      ...
      <div className={`relative bg-white rounded-3xl p-8 lg:p-10 shadow-2xl transform transition-all duration-500 group-hover:-translate-y-3 group-hover:${hoverRotations[index]} border-4 ${borderColors[index]} overflow-hidden`}>
        ...
        <div className="relative mb-8">
          ...
          <div className="relative bg-gradient-to-br from-[#fce001] to-[#fdb813] w-24 h-24 rounded-2xl flex items-center justify-center mx-auto shadow-xl transform group-hover:scale-110 group-hover:rotate-3 transition-all duration-300">
            <svg className="w-12 h-12 text-[#1a1a1a] drop-shadow-sm" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d={icons[step.icon as keyof typeof icons]} />
            </svg>
          </div>
        </div>
