...
export default function AppStoreButton({ href, icon, label, store }: AppStoreButtonProps) {
  return (
    <Link
      href={href}
      target="_blank"
    ...
    >
      ...
      <div className="relative z-10 flex items-center gap-3">
        {icon === "google" ? (
          <svg className="w-8 h-8 group-hover:text-[#1a1a1a] transition-colors duration-300" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="text-left group-hover:text-[#1a1a1a] transition-colors duration-300">
          ...
          <div className="text-base font-black leading-tight">{store}</div>
        </div>
      </div>
    </Link>
  );
