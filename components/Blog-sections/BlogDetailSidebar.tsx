...
export default function BlogDetailSidebar({
  ...
  return (
    <aside className="space-y-5 lg:pt-2">
      {/* Author card — Figma */}
      <div className="sticky top-28 rounded-[28px] border border-[#eceae4] bg-white p-5 shadow-[0_8px_28px_rgba(0,0,0,0.06)] sm:rounded-[32px] sm:p-6">
        <div className="mb-4 flex items-center gap-3.5">
          <span className="flex h-[52px] w-[52px] shrink-0 items-center justify-center rounded-full bg-[#FCE001] text-[15px] font-bold text-[#0b0b0b] shadow-[0_0_0_6px_rgba(252,224,1,0.25)]">
            {authorInitials || "TP"}
          </span>
          <div>
          ...
          <ul className="space-y-3.5">
            {tocItems.map((item) => (
              <li key={item.id}>
                ...
                >
                  <span className="mt-[0.45em] h-2 w-2 shrink-0 rounded-full bg-[#FCE001]" />
                  <span>{item.text}</span>
                </a>
              </li>
            ))}
