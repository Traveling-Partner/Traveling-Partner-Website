/**
 * Build Delivery mobile shell: solid yellow + card photos with
 * icons/text wiped from the top band of each card (labels come from HTML only).
 */
...
(async () => {
  ...
  // Wipe icon + title band in each card by cloning pixels from mid-card
  for (const card of CARDS) {
    ...
    const bandH = Math.round(ch * 0.32);
    ...
    // Sample donor row ~45% down the card (below labels)
    const donorLy = Math.min(ch - 1, Math.round(ch * 0.45));

    for (let ly = 0; ly < bandH; ly++) {
      for (let lx = 0; lx < cw; lx++) {
        const a = maskBuf[(ly * cw + lx) * 4 + 3];
        if (a < 128) continue;

        // Prefer donor pixel at same lx if inside mask, else search nearby
        let srcLy = donorLy;
        let found = false;
        for (let tryY = donorLy; tryY < ch && !found; tryY++) {
          if (maskBuf[(tryY * cw + lx) * 4 + 3] >= 128) {
            srcLy = tryY;
            found = true;
          }
        }
        ...
        const di = ((top + ly) * W + (left + lx)) * 4;
        const si = ((top + srcLy) * W + (left + lx)) * 4;
        data[di] = data[si];
        data[di + 1] = data[si + 1];
        data[di + 2] = data[si + 2];
        data[di + 3] = 255;
  ...
  await sharp(data, { raw: { width: W, height: H, channels: 4 } })
    .png()
    .toFile(path.join(dir, "services-mobile-delivery-blank.png"));

  console.log("→ services-mobile-delivery-blank.png");
})();
